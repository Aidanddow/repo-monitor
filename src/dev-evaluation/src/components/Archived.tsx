...
const Navbar: React.FC<Props> = props => {
  ...
  const unarchive = (e: React.MouseEvent<HTMLButtonElement> | undefined)=>{
    if(e){
      dispatch(actions.archiveRepo.request({id: +e.currentTarget.id, archived: false}));
    }
    
  }
  return (
   <div>
    <h4 className='text-start pb-3'>Archived Repositories</h4>
<TableContainer component={Paper}>
      <Table sx={{ minWidth: 650 }} aria-label="simple table">
        ...
        <TableBody>
        {archivedRepos.map(repo=><TableRow
              key={1}
            ...
            >
              ...
              <TableCell align="right"><Button variant='contained' size='small' id = {repo.id.toString()} onClick={unarchive}>
